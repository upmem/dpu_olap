...
public:
    ...
    template <typename T>
    void
    copy_from(const std::string &DstSymbol, const std::vector<std::vector<T>> &SrcBuffers, unsigned From)
    {
        struct dpu_set_t dpu;
        unsigned dpuIdx;

        DPU_FOREACH (cSet, dpu, dpuIdx) {
            DpuError::throwOnErr(dpu_prepare_xfer(dpu, (void *)SrcBuffers[dpuIdx+From].data()));
        }

        dpu_xfer_flags_t flags = async ? DPU_XFER_ASYNC : DPU_XFER_DEFAULT;
        DpuError::throwOnErr(dpu_push_xfer(cSet, DPU_XFER_TO_DPU, DstSymbol.c_str(), 0, sizeof(T), flags));
    }
    ...
    void copy(std::vector<std::vector<T>>& DstBuffers, const std::string& SrcSymbol,
              unsigned Offset, unsigned From) {
      if (DstBuffers.size() <= From) {
        DpuError::throwOnErr(DPU_ERR_INVALID_MEMORY_TRANSFER);
      }

      unsigned nrElements = DstBuffers[From].size();
      uint32_t nr_dpus;
      dpu_get_nr_dpus(cSet, &nr_dpus);
      for (auto buf = DstBuffers.begin() + From;
           buf != DstBuffers.begin() + From + nr_dpus; ++buf) {
        if (nrElements != buf->size()) {
          DpuError::throwOnErr(DPU_ERR_INVALID_MEMORY_TRANSFER);
        }
      }
      ...
      DPU_FOREACH (cSet, dpu, dpuIdx) {
          DpuError::throwOnErr(dpu_prepare_xfer(dpu, DstBuffers[dpuIdx + From].data()));
      }

      dpu_xfer_flags_t flags = async ? DPU_XFER_ASYNC : DPU_XFER_DEFAULT;
      DpuError::throwOnErr(dpu_push_xfer(cSet, DPU_XFER_FROM_DPU, SrcSymbol.c_str(), Offset, nrElements * sizeof(T), flags));
    }
...
public:
    ~DpuSet()
    {
        if (manageCSet or manageChildren) {
     ...
     */
    static DpuSet
    allocate(unsigned NrDpus = ALLOCATE_ALL, const std::string &Profile = "nrJobsPerRank=256")
    {
        struct dpu_set_t cSet;
    ...
    log(std::ostream &LogStream)
    {
        for (DpuSet *dpuSet : _dpus) {
            ...
            auto err = dpulog_read_for_dpu_(dpu, ostreamPrint, &LogStream);
            if (err != DPU_ERR_LOG_CONTEXT_MISSING) {
                DpuError::throwOnErr(err);
            }
        }
    }

    void
    exec_with_log(__attribute__((unused)) std::ostream &LogStream)
    {
#if ENABLE_LOG
        try {
            exec();
...
private:
    ...
    DpuSet(struct dpu_set_t CSet, bool ManageCSet = true, bool DetectChildren = true, bool ManageChildren = false)
...
private:
    ...
    {
        DpuSet set(CSet, false, true, true);
