...
static constexpr int64_t kMemcopyThreshold = 1024 * 1024;
...
inline void* wrap_memcpy(void* dst, const void* src, size_t n) {
  return memcpy(dst, src, n);
}

static std::vector<arrow::Future<void*>> parallel_memcopy(
    arrow::internal::ThreadPool* pool, uint8_t* dst, const uint8_t* src, int64_t nbytes,
    uintptr_t block_size=kMemcopyDefaultBlocksize, int num_threads=kMemcopyDefaultNumThreads) {
  assert(nbytes >= kMemcopyThreshold);
