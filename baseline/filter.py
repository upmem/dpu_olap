...
def with_datafusion(batches):
    ...
    result_batches = df.select(datafusion.col(
        "v") < datafusion.lit(1 << 30)).collect()
...
def with_polars(batches):
    ...
    # Note that filtering is single threaded
    # Multiple threads would be used if we had multiple expressions
    result = df.select([pl.col("v").filter(pl.col("v") < (1 << 30))])
    return result.to_arrow()


def with_arrow(batches):
    table = pa.Table.from_batches(batches)
    mask = cp.less(table.column("v"), 1 << 30)
    return cp.filter(table.column("v"), mask)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Benchmark Filter.')
    ...
    parser.add_argument('--batches', dest='batches', type=int, default=128,
                        help='number of batches with scale factor of 1 (default: 128)')
